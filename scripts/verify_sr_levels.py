...
def simulate_v2_monitor_trace():
    ...
    # Determine open time
    is_mcx = any(x in inst_key.upper() for x in ['CRUDE', 'NATURAL', 'GOLD', 'SILVER'])
    open_minute = 0 if is_mcx else 15

    # User provided 1-minute data from NSE_FO|64872
    base_time = datetime(2026, 2, 10, 9, open_minute)
    ...
    for i in range(len(df_1m)):
        ...
        df_completed = df_1m[
            (df_1m.index.time >= time(9, open_minute)) &
        ...
        # Resampling
        resampled_buckets = []
        anchor_1m = df_completed[df_completed.index.time == time(9, open_minute)]
