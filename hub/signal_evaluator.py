        ...
        for mode in active_modes:
            ...
            for side in ['CE', 'PE']:
                ...
                if gate_enabled:
                    ...
                    idx_high, idx_low = await self.indicator_manager.get_index_open_range(
                ...
                if not range_passed:
                    ...
                    if timestamp.timestamp() - last_range_log >= 60:
                        ...
                        if not breach_happened:
                            wait_msg = f"Waiting for Market Open {'High' if side == 'CE' else 'Low'} Breach"
                        else:
                            wait_msg = f"Price {idx_ltp:.2f} is not {'above' if side == 'CE' else 'below'} Market Open Low ({idx_low:.2f})"
                ...
                if self.monitor._evaluate_formula(entry_formula, eval_results):
                    if not range_passed:
                        last_range_log = getattr(
                            self.monitor, f'_last_range_wait_{side}_{mode}', 0)
                        if timestamp.timestamp() - last_range_log >= 60:
                            wait_msg = "Waiting for Market Open High Breach" if side == 'CE' else "Waiting for Market Open Low Breach"
                            logger.info(
                                f"V2: [{side}] Entry BLOCKED. {wait_msg}. Index: {idx_ltp:.2f} (Open Range: {idx_low:.2f} - {idx_high:.2f})"
                            )
                            setattr(self.monitor,
                                    f'_last_range_wait_{side}_{mode}',
                                    timestamp.timestamp())
