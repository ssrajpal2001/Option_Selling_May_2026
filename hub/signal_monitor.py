...
class SignalMonitor:
    ...
    def _get_market_open_minute(self):
        """Returns the market open minute (0 for MCX, 15 for others)."""
        strat_start = self.orchestrator.get_strat_cfg("sell.start_time")
        if not strat_start:
            strat_start = self.orchestrator.get_strat_cfg("buy.start_time")

        if strat_start:
            try:
                parts = str(strat_start).split(':')
                if len(parts) >= 2:
                    return int(parts[1])
            except: pass

        # 1. Priority: instrument-specific INI config start_time
        inst_start = self.config_manager.get(self.orchestrator.instrument_name, 'start_time')
        if inst_start:
            try:
                # Handle HH:MM and HH:MM:SS
                if len(inst_start.split(':')) == 2:
                    t = datetime.strptime(inst_start, '%H:%M').time()
                else:
                    t = datetime.strptime(inst_start, '%H:%M:%S').time()
                return t.minute
            except: pass

        return 0 if self._is_mcx() else 15

    def _is_mcx(self):
        return self.orchestrator.is_mcx
    ...
    async def _check_market_open_range_breach(self, timestamp):
        await self.gate_manager.check_market_open_range_breach(timestamp)
    ...
    async def check_crossover_breach(self, timestamp, current_atm):
        ...
        """
        # --- V3 SILENCE RULE ---
        # If V3 side is active, we completely skip V2 crossover breach processing
        # to avoid redundant log noise and CPU usage.
        # Note: We check specifically for the instrument-level V3 mode.
        v3_on = self.orchestrator.json_config.get_value(f"{self.orchestrator.instrument_name}.v3_mode")
        if v3_on is None:
            v3_on = self.orchestrator.json_config.get_value("NIFTY.v3_mode")
        ...
        async with self._breach_lock:
            ...
            ref_mode = active_modes[0] if active_modes else 'sell'

            # 0. Global Market Open Breach Gate Check (if enabled)
            gate_enabled = self._get_user_setting('range_915_gate_enabled', bool, fallback=True, mode=ref_mode)
            if gate_enabled:
                await self._check_market_open_range_breach(timestamp)
            else:
            ...
            # 2. EVALUATION & STATUS LOG (skip BUY evaluation when V3 mode is on)
            if active_modes and self.state_manager.dual_sr_monitoring_data:
            ...
            if active_modes and (is_log_minute or is_switching) and last_status_log_ts != timestamp:
                ...
                if monitor_data:
                    ...
                    if ce_data and pe_data:
                        ...
                        # Market Open Range Info
                        is_mcx = self._is_mcx()
                        idx_ltp = (self.state_manager.spot_price if is_mcx else self.state_manager.index_price) or 0.0
                        gate_enabled = self._get_user_setting('range_915_gate_enabled', bool, fallback=True, mode=ref_mode)
                        if gate_enabled:
                            idx_high, idx_low = await self.indicator_manager.get_index_open_range(self.orchestrator.index_instrument_key, timestamp)
                            range_str = f"OpenR({idx_low:.1f}-{idx_high:.1f})" if idx_high else "OpenR(Wait)"
                            status = 'NONE'
                        ...
                        if hasattr(self.orchestrator, 'sell_manager'):
                            ...
                            parts = []
                            if getattr(sm, 'ce_placed', False) and getattr(sm, 'sell_ce_strike', None) is not None:
                            ...
                            if getattr(sm, 'pe_placed', False) and getattr(sm, 'sell_pe_strike', None) is not None:
    ...
    async def format_state(self, side_data, direction, formula, timestamp):
        """Helper to format the side state for logging, respecting the market open gate."""
    ...
    async def _check_crossover_for_all_sides(self, timestamp, force_indicator_refresh=False, active_modes=['buy']):
        ...
        # START OF DAY LOGIC:
        # Trade will start ONLY AFTER market open candle is formed (at Market Open + 1 minute).
        open_min = self._get_market_open_minute()
        if current_minute_start.hour == 9 and current_minute_start.minute <= open_min:
            # It's before or exactly at open. The first candle hasn't finalized yet.
            return
        ...
        # Historical pattern checking is ALWAYS allowed to support market open analysis.
        ...
        anchor_ts = last_1m_completed_ts.replace(hour=9, minute=open_min, second=0, microsecond=0)
        ...
        is_rule_1 = (last_1m_completed_ts.hour == 9 and last_1m_completed_ts.minute == open_min)
