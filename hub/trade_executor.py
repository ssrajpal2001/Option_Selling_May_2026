...
class TradeExecutor:
    ...
    async def execute_trade_v2(self, direction, signal_instrument_key, signal_ltp, strike_price, timestamp, signal_strike, strategy_log="", user_id=None, entry_type='BUY', quantity_multiplier=1):
        ...
        # All trades: recalculate trade strike from anchor price (Futures for MCX, Index for others).
        ...
        if anchor_price and anchor_price > 0:
            trade_strike = int(round(anchor_price / strike_interval) * strike_interval)
            price_type = "FUTURES" if self.orchestrator.is_mcx else "INDEX"
            logger.info(f"V2: {entry_type} trade strike from {price_type} ({anchor_price:.2f}) → {trade_strike} [Signal strike was: {strike_price}]")
        else:
            trade_strike = strike_price
            logger.warning(f"V2: Anchor price unavailable, falling back to signal strike {strike_price}")
