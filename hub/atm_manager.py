...
class AtmManager:
    ...
    def set_ready(self):
        # Synchronize contract data from DataManager if available
        if self.data_manager and self.data_manager.all_options:
            self.all_contracts = self.data_manager.all_options
            self.near_expiry_date = self.data_manager.near_expiry_date
            self.monthly_expiries = self.data_manager.monthly_expiries
            self._build_contract_lookup_table()

        self.expiry_manager = ExpiryManager(self.all_contracts, self.near_expiry_date, self.monthly_expiries, config_manager=self.config)
        self.sub_manager = SubscriptionManager(self.websocket, self.config, self.data_manager, self.orchestrator)
        self._is_ready = True
        logger.debug(f"AtmManager initialized for {self.instrument_name}. Lookup table size: {len(self.contract_lookup)}")
    ...
    async def _handle_spot_price_update(self, data):
        ...
        # PriceFeedHandler already updates state_manager.spot_price (Futures)
        # or state_manager.index_price (Index) before publishing this event.
    ...
    def find_instrument_key_by_strike(self, strike_price, option_type, expiry_date):
        ...
        # PERFORMANCE: Backtest Strike Lookup Cache
        if self.orchestrator and self.orchestrator.is_backtest:
            if not hasattr(self, '_bt_key_cache'): self._bt_key_cache = {}
            k = (float(strike_price), api_type, target)
            if k in self._bt_key_cache: return self._bt_key_cache[k]

        expiry_strikes = self.contract_lookup.get(target, {})
        contract = expiry_strikes.get(float(strike_price), {}).get(api_type)
        res = contract.instrument_key if contract else None

        if self.orchestrator and self.orchestrator.is_backtest and res:
            self._bt_key_cache[k] = res
        return res
    ...
    def _determine_default_expiries(self, context_date):
        # Prioritize instrument-specific config over global settings
        sig_mode = self.config.get(self.instrument_name, 'signal_expiry', fallback=self.config.get('settings', 'signal_expiry', fallback='CURRENT_WEEK')).upper()
        trd_mode = self.config.get(self.instrument_name, 'trade_expiry_type', fallback=self.config.get('settings', 'trade_expiry_type', fallback='WEEKLY')).upper()
