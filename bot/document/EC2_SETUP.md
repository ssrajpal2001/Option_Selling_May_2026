...
## 10. Force Stopping the Bot

If the UI says **"Bot already running"** but you cannot start it, or if a process is orphaned, run these commands in the terminal:

### Kill all Bot Instances
This stops all active trading bot processes:
```bash
pkill -f "python3 main.py"
```

### Kill the Web Server
This stops the dashboard/API server:
```bash
pkill -f "uvicorn web.server:app"
```

### Cleanup PID Files
If the UI still thinks it's running after the kill command, delete the lock file:
```bash
rm config/bot.pid
```
