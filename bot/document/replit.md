...
- Tables: users, data_providers, client_broker_instances, trade_history, order_failures, audit_log, broker_change_requests
...
- Client submits via Settings → "Request Broker Change" (validates: current broker exists, requested not already configured, no bot running, no pending request)
- Admin sees pending requests on Overview dashboard + Client Detail page (approve/deny)
- On approval: old broker instance soft-disabled (`status='removed'`), running bots stopped; client can then configure the new broker
- On denial: request marked denied, no changes made
- `broker_change_requests` table tracks full history (pending → approved/denied)
- All queries for active broker instances exclude `status='removed'`
...
- `/api/admin/*` — data provider config, client management, overview, bot monitor, broker change request approve/deny
- `/api/client/*` — broker setup, bot start/stop/status, trade history, broker change requests
