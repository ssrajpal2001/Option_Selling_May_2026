...
class TickProcessor:
    ...
    def _get_buy_start_time(self):
        ...
        try:
            raw = self.orchestrator.get_strat_cfg("buy.start_time")
            if raw:
                parts = str(raw).split(":")
    ...
    async def process_tick(self, backtest_previous_tick=None, backtest_current_tick=None):
        ...
        """
        # In multi-tenant live mode, we want the heartbeat and spot price updates
        # to proceed even if the orchestrator is not 'active' (pre-market wait).
        # We only block if the system is truly idle and not in backtest.
    ...
    async def process_tick_v2(self, backtest_previous_tick=None, backtest_current_tick=None):
        ...
        # TRADING TOGGLE CHECK (Commercial Requirement)
        ...
        if not self.orchestrator.is_backtest and user_id:
            ...
            if os.path.exists(sq_file):
                try:
                    logger.info(f"[Client {user_id}] SQUARE OFF ALL signal detected.")
                    for session in self.orchestrator.user_sessions.values():
                        await session.position_manager.close_all_positions(reason="MANUAL_SQUARE_OFF")

                    sm = getattr(self.orchestrator, 'sell_manager', None)
                    if sm and not sm.strangle_closed:
                        await sm.close_all(timestamp, reason="MANUAL_SQUARE_OFF")

                    os.remove(sq_file) # Clear signal
                except Exception as e:
        ...
        if not futures_price:
            # Fallback to Index price if Futures is not available
            futures_price = index_price
            if futures_price:
                if not hasattr(self, '_last_futures_warn_time') or (now_ts - self._last_futures_warn_time > 300):
                    logger.info(f"V2: [{self.orchestrator.instrument_name}] Futures price missing. Falling back to Index price {futures_price} for ATM calculation.")
                    self._last_futures_warn_time = now_ts
            else:
                if not hasattr(self, '_last_data_warn_time') or (now_ts - self._last_data_warn_time > 60):
                    logger.warning(f"V2: [{self.orchestrator.instrument_name}] Neither Futures nor Index price available. Tick skipped.")
                    self._last_data_warn_time = now_ts
                return
        ...
        # Gated by absolute exchange open time (09:15) to ensure historical data readiness
        exchange_open = self.orchestrator.get_exchange_open_time(timestamp)
        if not self.orchestrator.is_backtest and self.orchestrator.data_recorder and timestamp >= exchange_open:
        ...
        trading_permitted = getattr(self, 'trading_enabled', True) or self.orchestrator.is_backtest
        ...
        if hasattr(self.orchestrator, 'sell_manager'):
            ...
            if isinstance(sm, SellManagerV3):
                # V3 integrated logic
                if not sm.strangle_closed:
                    sell_ticks = {}
                    for k, v in self.state_manager.option_prices.items():
                        if v: sell_ticks[k] = {'ltp': float(v)}

                    # Only process ticks for entry/management if trading is permitted or if it's an exit check
                    # Actually sm.on_tick handles its own state, but we skip if closed.
                    # We should allow on_tick if strangle is open to process exits even if trading_permitted is False?
                    # No, the toggle-off already closed it. So we only run if permitted.
                    if trading_permitted:
                        await sm.on_tick(sell_ticks, timestamp)
            elif not sm.strangle_closed and (sm.ce_candidates or sm.pe_candidates or sm.ce_placed or sm.pe_placed):
                if trading_permitted:
