...
class CSVDataFeeder:
    ...
    def load_data(self, target_date=None):
        ...
        try:
            ...
            # 0. Proactive Search: If default tick_data_log.csv is missing, look for ANY relevant CSV
            if not os.path.isfile(self.file_path):
                candidates = [
                    self.file_path,
                    os.path.join(self._get_project_root(), 'tick_data_log.csv'),
                    os.path.join(self._get_project_root(), 'mini_market_data.csv'),
                    os.path.join(self._get_project_root(), 'Selling_Using_C', os.path.basename(self.file_path))
                ]
                ...
                for cand in candidates:
                    if cand and os.path.isfile(cand):
                        logger.info(f"Using backtest data from: {cand}")
                        self.file_path = cand
                        break
            ...
            # Robust timestamp parsing for ISO strings and other formats
            # We use utc=True to ensure we get a DatetimeIndex/Series even with timezone offsets
            long_df['timestamp'] = pd.to_datetime(long_df['timestamp'], errors='coerce', utc=True)
            long_df.dropna(subset=['timestamp'], inplace=True)

            self.data = long_df

            # Ensure we are in Asia/Kolkata
            if not self.data.empty:
                # After pd.to_datetime(..., utc=True), it is definitely datetimelike
