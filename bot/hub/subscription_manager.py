...
class SubscriptionManager:
    ...
    async def perform_resubscription(self, atm_strike, protected_keys, signal_expiry_date, contracts_dict, find_contracts_func):
        ...
        v3_mode = self.orchestrator.json_config.get_value(f"{self.orchestrator.instrument_name}.v3_mode")
        if v3_mode is None:
            v3_mode = self.orchestrator.json_config.get_value("NIFTY.v3_mode")

        if str(v3_mode).lower() == 'true':
            num_strikes = max(num_strikes, 10) # V3 needs ATM +/- 10
        ...
        if not is_backtest and final_keys:
            # 1. Global Registration (High Availability Feeds: Upstox + Dhan)
            # We register handlers for both legacy protobuf (Upstox) and normalized (any provider)
            for k in final_keys:
                await tick_dispatcher.register(k, self.orchestrator.price_feed_handler.handle_tick_dispatched)
                await tick_dispatcher.register(k, self.orchestrator.price_feed_handler.handle_tick_dispatched_normalized)

            # Universal Keys are used for both global feeds
            self.websocket.subscribe(final_keys, mode='full')

            # 2. Individual Broker Subscriptions (DISABLED for market data as per requirement)
            # Ticks are now handled globally via Admin feeds.
            # No user-specific data registration is required here anymore.
        return final_keys
