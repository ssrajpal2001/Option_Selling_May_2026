...
class PositionManager:
    ...
    async def _evaluate_barriers(self, direction, pos_data, key, timestamp, mode, primary_tf, profit, global_target, force_update=False):
        ...
        # Monotonic update
        anchor = self.indicator_manager._get_market_open_time(timestamp)
        ...
        if entry_ts:
            ...
            if ohlc_df is not None and not ohlc_df.empty:
                ...
                if not relevant_past.empty:
                    ...
                    # Start from Market Open of the current day
                    session_start = self.indicator_manager._get_market_open_time(timestamp_aware)
    ...
    async def close_all_positions(self, reason="MANUAL"):
        """Closes all active trades for this session immediately."""
        timestamp = self.orchestrator._get_timestamp()
        for side in ['CALL', 'PUT']:
            if self.state_manager.is_in_trade(side):
                pos = self.state_manager.call_position if side == 'CALL' else self.state_manager.put_position
                ltp = self.state_manager.get_ltp(pos['instrument_key']) or pos.get('ltp', 0)
                await self._exit_trade(side, ltp, timestamp, reason=reason)
    ...
    async def _exit_trade(self, side, ltp, timestamp, reason='Stop Loss', is_sl_exit=False):
        ...
        # Reset Market Open Breach Gate ONLY on hard SL (not TSL) as per user distinction
        # User: "if ltrade is runnign and sl is hit .it will again wait for breach"
        ...
        if is_hard_sl:
            if gate_enabled:
                logger.info(f"V2 MGMT: [{side}] Stop Loss detected. Resetting Market Open Range Breach flags for re-evaluation.")
                self.state_manager.range_915_breached_up = False
                self.state_manager.range_915_breached_down = False
        elif gate_enabled:
            logger.info(f"V2 MGMT: [{side}] Exit ({reason}). Preserving Market Open Breach flags for potential re-entry.")
