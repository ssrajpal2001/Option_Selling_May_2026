...
class UserSession:
    ...
    def __init__(self, user_id, email, instrument_name, orchestrator, strategy_config=None):
        ...
        # 5. Isolated SellManagerV3 (if enabled)
        self.sell_manager = None
        v3_mode = orchestrator.json_config.get_value(f"{instrument_name}.v3_mode")
        if v3_mode is None: v3_mode = orchestrator.json_config.get_value("NIFTY.v3_mode")

        if str(v3_mode).lower() == 'true':
            from hub.sell_manager_v3 import SellManagerV3
            self.sell_manager = SellManagerV3(self)
            logger.info(f"[{instrument_name}] SellManagerV3 initialized for User: {email}")
