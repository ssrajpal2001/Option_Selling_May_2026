...
from .tick_dispatcher import tick_dispatcher
import asyncio
...
class LiveOrchestrator(BaseOrchestrator):
    def __init__(self, *args, **kwargs):
        ...
        # In multi-instrument client mode, if the JSON only contains NIFTY,
        # we fall back to NIFTY settings for V3 mode check and other strategy params.
    ...
    async def handle_add_to_watchlist(self, data):
        ...
        if instrument_key and instrument_key not in self.subscribed_instruments:
            ...
            await tick_dispatcher.register(instrument_key, self.price_feed_handler.handle_tick_dispatched)
    ...
    async def handle_remove_from_watchlist(self, data):
        ...
        if instrument_key and instrument_key in self.subscribed_instruments:
            ...
            await tick_dispatcher.unregister(instrument_key, self.price_feed_handler.handle_tick_dispatched)
