    ...
    async def _run_backtest_loop(self):
        ...
        for current_date in dates_to_run:
            ...
            for timestamp, group in data_feeder.data.groupby('timestamp'):
                ...
                if elapsed_ms > 50: # Log only slow ticks (>50ms) to avoid log spam
                    logger.debug(f"[Backtest] Slow tick at {timestamp}: {elapsed_ms:.2f}ms")
                ...
                for trade in all_trades:
                    ...
                    if temp_broker_for_reporting and 'contract' in trade:
                        try:
                            ...
                            # Handle various broker-specific symbol construction methods
                            if hasattr(temp_broker_for_reporting, 'construct_zerodha_symbol'):
                                symbol = temp_broker_for_reporting.construct_zerodha_symbol(trade['contract'], signal_expiry)
                            elif hasattr(temp_broker_for_reporting, 'construct_dhan_symbol'):
                                symbol = temp_broker_for_reporting.construct_dhan_symbol(trade['contract'])
                            else:
                                symbol = trade.get('instrument_key', 'N/A')
                        except Exception as e:
