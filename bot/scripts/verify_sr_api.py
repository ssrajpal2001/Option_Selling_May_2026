...
async def verify_sr_api(target_instrument, target_strike, target_type, target_expiry, target_date_str, timeframe_minutes, direct_key=None):
    ...
    # Determine open time
    is_mcx = any(x in target_instrument.upper() for x in ['CRUDE', 'NATURAL', 'GOLD', 'SILVER'])
    open_minute = 0 if is_mcx else 15
    open_time = time(9, open_minute)

    # Filter strictly from Market Open today
    df_1m = ohlc_1m[
        (ohlc_1m.index.date == target_date) &
        (ohlc_1m.index.time >= open_time)
    ].copy()

    if df_1m.empty:
        print(f"Error: No data available after {open_time} on {target_date}")
        await api_manager.close()
        return
    ...
    # Rule #1: Market Open 1m candle
    anchor_1m = df_1m[df_1m.index.time == open_time]
