...
class RestApiClient:
    ...
    async def _request(self, method, endpoint, params=None, retry=True, silence_error=False, timeout=15):
        try:
            ...
            if should_retry:
                retry_delay = 5 if status == 429 else 1
                logger.info(f"Attempting to switch client and retry API call to {endpoint} in {retry_delay}s...")
    ...
    async def get_historical_candle_data(self, instrument_key, interval, to_date, from_date):
        ...
        # In backtest mode, never attempt to merge with "real" intraday data
        if to_date_str == today_str and not self.backtest_mode:
