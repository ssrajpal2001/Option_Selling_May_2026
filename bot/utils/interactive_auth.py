...
def get_new_token_interactively(config_manager, credentials_section):
    ...
    try:
        ...
        if new_token:
            ...
            # Sync with algosoft.db for UI
            if credentials_section.lower().startswith('upstox'):
                try:
                    ...
                    logger.info("Upstox access token synced with UI database.")
                except Exception as db_err:
                    logger.warning(f"Failed to sync Upstox token to UI database: {db_err}")
