...
class SupportResistanceCalculator:
    ...
    async def get_sr_status_shared(data_manager, inst_key, timeframe_minutes, timestamp, ohlc_1m):
        """
        Calculates S&R status by replaying 1-minute history since market open.
        ...
        is_mcx = any(x in str(inst_key).upper() for x in ['MCX', 'CRUDE', 'NATURAL', 'GOLD', 'SILVER'])
        open_minute = 0 if is_mcx else 15

        df_1m = ohlc_1m[
            (ohlc_1m.index.date == today) &
            (ohlc_1m.index.time >= time(9, open_minute)) &
        ...
        if not df_1m.empty:
            # Reindex to fill any gaps in 1-minute data between open and boundary_ts-1m
            # This ensures bucket calculations (iloc) remain synchronized with the clock.
            anchor_ts = boundary_ts.replace(hour=9, minute=open_minute)
        ...
        if df_1m.empty or pd.isna(df_1m.iloc[0]['high']):
            # FALLBACK: If history doesn't start at open, calculation will fail.
            # However, we've already tried fetching from API in _get_robust_ohlc.
            # If we are here, it means even API doesn't have open candle for this strike.
            # This can happen if the strike wasn't active.
            return None, None, False, False, None, None, None, None

        # Rule #0.1: Force anchor data if missing
        if df_1m.index[0].time() > time(9, open_minute):
        ...
        # Rule #1: Anchor at Market Open (Mandatory start of day)
        ...
        for i in range(len(df_1m)):
            ...
            if timeframe_minutes > 1 and (calc.states[inst_key]['current_phase'] != 'INITIAL_TREND_ESTABLISHMENT' or is_at_boundary):
                # Strategy Requirement: TF buckets must be clock-aligned to market open anchor.
